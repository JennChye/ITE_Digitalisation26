...
export default function PositiveLearningDashboard({ onOpenCollection = () => undefined }: { onOpenCollection?: () => void }) {
  ...
  return (
    <section className="mt-7" aria-label="Private learning progress">
      ...
      {state.settings.badgesEnabled && <section className="mt-6 rounded-[1.75rem] border border-[#dce8d1] bg-[#fffdf5] p-5 shadow-[0_10px_24px_rgba(36,79,54,0.08)] sm:p-6" aria-labelledby="achievements-title"><div className="flex items-start gap-4"><div className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#f8e6a2] text-[#72591c] shadow-[0_4px_0_#ddc56d]"><Award className="size-5" aria-hidden="true" /></div><div><p className="text-xs font-extrabold uppercase tracking-[0.15em] text-[#8c7025]">Positive progress</p><h2 id="achievements-title" className="font-display mt-1 text-3xl tracking-[-0.05em] text-[#173f2e]">Achievements</h2><p className="mt-2 text-sm leading-6 text-[#5d7465]">Explore at your own pace. There are no daily streaks or deadlines.</p></div></div>{notice && <p role="status" className="mt-4 rounded-xl bg-[#e7f2df] p-3 text-sm font-extrabold text-[#2c6541]">{notice}</p>}<div className="mt-5 grid grid-cols-2 gap-3">{earned.map((achievement) => <AchievementCard key={achievement.id} achievement={achievement} />)}{locked.map((achievement) => <AchievementCard key={achievement.id} achievement={achievement} />)}</div><button type="button" onClick={onOpenCollection} className="mt-4 min-h-12 w-full rounded-xl border border-[#b9d8b4] bg-[#edf6e7] px-4 text-sm font-extrabold text-[#286142] active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#2c7049]">View my badge collection</button><div className="mt-5 grid grid-cols-2 gap-3 rounded-2xl bg-[#f4f0e4] p-4 text-center"><div><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#6a806f]">Badges earned</p><p className="font-display mt-1 text-3xl text-[#214b35]">{earned.length}/6</p></div><div><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#6a806f]">Meals logged</p><p className="font-display mt-1 text-3xl text-[#214b35]">{logs.length}</p></div><div><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#6a806f]">Meals explored</p><p className="font-display mt-1 text-3xl text-[#214b35]">{uniqueMeals}</p></div><div><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#6a806f]">Swaps viewed</p><p className="font-display mt-1 text-3xl text-[#214b35]">{state.viewedRecommendationIds.length}</p></div></div></section>}
    </section>
  );
}
